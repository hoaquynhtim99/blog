        ...
        if (!in_array($fbcmtid, $session_fbcmtids)) {
            ...
            if (!empty($BL->setting['emailWhenComment'])) {
                ...
                if (!empty($emails)) {
                    ...
                    $message = sprintf($nv_Lang->getModule('email_ncomment_b'), $row['title'], $link, $link);
                    nv_sendmail([$global_config['site_name'], $global_config['site_email']], $emails, $nv_Lang->getModule('email_ncomment_s'), $message);
                }
            }
